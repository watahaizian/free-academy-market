  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={isFavorite.includes(item.item_id) ? "お気に入りから削除" : "お気に入りに追加"}
      >
        <Heart
          className={`${isFavorite.includes(item.item_id) ? 'text-red-500 fill-red-500' : 'text-white'} transition-colors`}
        />
      </button>
    </div>
  )
